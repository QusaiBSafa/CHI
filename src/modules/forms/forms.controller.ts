...
export class FormsController {
   ...
   */
  async archiveForm(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id } = request.params as { id: string };
      ...
      const archived = await this.service.archive(form.formId);

      return reply.status(200).send({
        success: true,
        data: this.formatForm(archived),
      });
    } catch (error: any) {
      return reply.status(400).send({
        success: false,
  ...
  /**
   * Validate sample submission (dry-run)
   * POST /admin/forms/:id/validate
   */
  async validateSubmission(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { id } = request.params as { id: string };
      const { sampleData } = request.body as { sampleData: Record<string, any> };
      ...
      const errors = await this.service.validateSampleSubmission(form.formId, sampleData);
      ...
      return reply.status(400).send({
        success: false,
        message: error.message || 'Failed to validate submission',
      });
    }
  }
