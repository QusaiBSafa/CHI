...
export async function formsRoutes(app: FastifyInstance) {
  ...
  // Archive form
  app.post('/admin/forms/:id/archive', {
    preHandler: requireAdmin,
    schema: {
      tags: ['Admin - Forms'],
    ...
    return controller.archiveForm(request, reply);
  });

  // Validate sample submission
  app.post('/admin/forms/:id/validate', {
    preHandler: requireAdmin,
    schema: {
      tags: ['Admin - Forms'],
      description: 'Dry-run validate a sample submission against form definition',
      security: [{ bearerAuth: [] }],
      params: {
        type: 'object',
        properties: {
      ...
      body: {
        ...
        properties: {
          sampleData: { 
            type: 'object', 
            description: 'Sample submission data to validate',
    ...
    return controller.validateSubmission(request, reply);
  });
